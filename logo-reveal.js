...
function fitLogo(){ if(!curPanels) return;
  ...
  const availW=stage.clientWidth*(mobile?1.0:0.9), availH=Math.max(150,stage.clientHeight*(mobile?0.62:0.46));
...
function glitchText(el, text, opts) {
  ...
  return new Promise((res) => {
    (function frame() {
      ...
      // On the final frame, force plain text: the done-check counts a char that is
      // still in its brief .hot settle window (t-lockAt<120) as done, so the last
      // chars ("build") could otherwise freeze mid-glow. Clean text = no stuck hot.
      if (!done) { el.innerHTML = html; requestAnimationFrame(frame); }
